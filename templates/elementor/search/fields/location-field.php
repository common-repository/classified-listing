...
if ( $geo_location ) :
	...
	<?php if ( isset( $settings['geo_location_range'] ) && $settings['geo_location_range'] ) { ?>
		<div class=" form-group ws-item ws-location rtcl-range-slider-field col-sm-6 ">
			<?php $radius_placeholder = sprintf( /* translators: radius unite */ esc_html__( 'Radius (%1$s)', 'classified-listing' ), $rs_data['units'] ?? '' ); ?>
			<?php if ( $settings['fields_label'] ) { ?>
	...
	<div class="form-group ws-item ws-location col-sm-6 col-12">
		...
		<?php if ( $style === 'suggestion' ) { ?>
			<div class="location-field-wrapper">
				<input type="text" data-type="location" class="rtcl-autocomplete rtcl-location form-control" placeholder="<?php echo esc_html( Text::get_select_location_text() ); ?>" value="<?php echo $selected_location ? esc_attr($selected_location->name) : ''; ?>">
