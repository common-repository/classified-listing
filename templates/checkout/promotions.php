...
<table id="rtcl-checkout-form-data"
	...
	<tr>
		...
		<th><?php printf( /* translators: Price symbol */ esc_html__( 'Price [%1$s %2$s]', 'classified-listing' ), esc_html( $currency ), esc_html( $currency_symbol ) ); ?></th>
	</tr>
	<?php
		...
		foreach ( $pricing_options as $pricing ) :
			...
			<tr>
				...
				<td class="rtcl-pricing-visibility"
					...
					<?php
					printf( '<span>%s</span>', esc_html( sprintf( /* translators: Days */ _n( '%s Day', '%s Days', absint( $visible ), 'classified-listing' ), esc_html( number_format_i18n( absint( $visible ) ) ) ) ) );
				...
				<td class="rtcl-pricing-price text-right"
					data-label="<?php printf( /* translators: Price symbol */ esc_html__( 'Price [%1$s %2$s]:', 'classified-listing' ), esc_html( $currency ), esc_html( $currency_symbol ) ); ?>">
					<?php
					/* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */
					echo Functions::get_payment_formatted_price( $price );
					?>
				</td>
			</tr>
		<?php endforeach;
