		...
		foreach ( $rows as $row ) {
			...
			if ( ! empty( $postarr ) ) {
				...
				if ( ! is_wp_error( $post_id ) ) {
					...
					if ( ! is_wp_error( $cat_id ) && is_array( $cat_id ) ) {
						wp_set_object_terms( $post_id, absint( $cat_id['term_id'] ), rtcl()->category );
					}
					if ( ! is_wp_error( $loc_id ) && is_array( $loc_id ) ) {
						wp_set_object_terms( $post_id, absint( $loc_id['term_id'] ), rtcl()->location );
					}
				...
				if ( is_file( $filename ) ) {
					...
					if ( $row_count > apply_filters( 'rtcl_import_listings_limit', 101 ) ) {
						...
						if ( ! empty( $title_row ) ) {
							...
							ob_start();
							?>
							<form class="rtcl-listings-import-mapping-form" id="rtcl-listings-import-mapping-form" name="rtcl-listings-import-mapping-form"
								  method="post">
								<header>
									...
									<table class="rtcl-importer-mapping-table">
										...
										foreach ( $title_row as $index => $title ) {
											...
											<tr>
												...
												<td>
													<select class="rtcl_map_to" name="map_to[<?php echo esc_attr( $index ); ?>]">
														<option value=""><?php _e( 'Do not import', 'classified-listing' ); ?></option>
														<?php
