...
class Breadcrumb
{
	...
	protected function add_crumbs_listings() {
		...
		$category = get_query_var( '__cat' ) ? get_query_var( '__cat' )
			: ( isset( $_GET['rtcl_category'] ) ? sanitize_text_field( $_GET['rtcl_category'] ) : '' ); /* phpcs:ignore WordPress.Security.NonceVerification.Recommended */
		if ( ! empty( $category ) ) {
			$cat_term  = get_term_by( 'slug', $category, rtcl()->category );
			$term_name = $cat_term->name ?? '';
			$term_link = get_term_link( $cat_term, rtcl()->category );
			$this->add_crumb( $term_name, $term_link );
		}

		$location = get_query_var( '__loc' ) ? get_query_var( '__loc' )
			: ( isset( $_GET['rtcl_location'] ) ? sanitize_text_field( $_GET['rtcl_location'] ) : '' ); /* phpcs:ignore WordPress.Security.NonceVerification.Recommended */
		if ( ! empty( $location ) ) {
			$loc_term  = get_term_by( 'slug', $location, rtcl()->location );
			$term_name = $loc_term->name ?? '';
			$term_link = get_term_link( $loc_term, rtcl()->location );
			$this->add_crumb( $term_name, $term_link );
		}
	}
